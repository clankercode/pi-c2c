 ...
 *
 * Cross-machine relay DMs ALSO use a `#`-suffixed `to_alias`:
 * `<recipient-relay-alias>` = `<name>#<12-hex-host-hash>` (see
 * `src/relay.ts:deriveRelayAlias`). Those are direct messages that
 * must reply via `c2c_pi_send`, not `c2c_pi_send_room`.
 *
 * Disambiguate by suffix shape: a 12-lowercase-hex suffix is the
 * relay host hash (generated by `computeHostHash`); anything else is
...
export function isRoomMessage(msg: C2cMessage): boolean {
  ...
  // Relay host-hash suffix is exactly 12 lowercase hex chars.
  if (/^[0-9a-f]{12}$/.test(suffix)) return false;
  return true;
}
